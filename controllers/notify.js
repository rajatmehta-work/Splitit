    ...
    jwt.verify(req.token, process.env.JWT_SECRET, (err, id) => {
        if (err) {
            ...
            const promise1 = new Promise((resolve, reject) => {
                ...
                .then((groupNames) => {
                    const query1 =
                        "select  sp_users.name,sp_friend_requests.request_id  from sp_users inner join sp_friend_requests on sp_users.id=sp_friend_requests.request_id where sp_friend_requests.uid=" +
                        curr_user;
                    const query2 =
                        "select friends from sp_users where id=" + curr_user;
